...
namespace duisharp {
	...
	class DUISHARP_API CUIMarkup
	{
		friend class CUIMarkupNode;
	public:
		CUIMarkup(LPCTSTR pstrXML = NULL);
		~CUIMarkup();
	...
	class DUISHARP_API CUIMarkupNode
	{
		friend class CUIMarkup;
	private:
		CUIMarkupNode();
		CUIMarkupNode(CUIMarkup* pOwner, int iPos);

	public:
		bool IsValid() const;

		CUIMarkupNode GetParent();
		CUIMarkupNode GetSibling();
		CUIMarkupNode GetSibling(LPCTSTR pstrName);
		CUIMarkupNode GetChild();
		CUIMarkupNode GetChild(LPCTSTR pstrName);
